// CopyFile.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
bool read_file_using_memory_map()
{
	...
	if (!SUCCEEDED(StringCbPrintfW(
		...
		buf)))
	{
		print("err ] can not creat file name");
		free(buf);
		return false;
	}
	free(buf); buf = NULL;
	...
	{
		print("err ] CreateFile(%s) failed, gle = %u", file_name, GetLastError());
		return false;
	}
	...
	)
{
	...
	do
	{
		...
		*/
		...
		// write to dst
		if (!WriteFile(dst_handle, buf, bytes_read, &bytes_written, NULL))
		{
